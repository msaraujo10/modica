export default function Sobre() {
    return (
        <div className="py-16">
            <div className="container mx-auto px-4">
                <h1 className="text-3xl font-bold text-blue-900 mb-6">Sobre a DiMod Clean</h1>
                <div className="prose prose-blue max-w-3xl">
                    <p>
                        Fundada por <strong>Paula Cristina</strong>, a DiMod Clean nasceu do cuidado
                        e atenção que Paula dedica aos seus próprios apartamentos no Airbnb em Copacabana.
                    </p>
                    <p>
                        Com anos de experiência na gestão de hospedagens, ela percebeu que a limpeza impecável
                        é o segredo para manter avaliações 5 estrelas e hóspedes satisfeitos.
                    </p>
                    <p>
                        Hoje, oferecemos esse mesmo padrão de excelência para outros anfitriões da região.
                    </p>
                </div>
            </div>
        </div>
    );
}